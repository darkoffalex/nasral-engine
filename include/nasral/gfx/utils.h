...
namespace nasral::gfx
{
    ...
    using GeometryData = std::tuple<std::vector<Vertex>, std::vector<uint32_t>>;

    inline GeometryData gen_quad_geometry(const float size = 1.0f)
    {
    ...
    inline GeometryData gen_cube_geometry(const float size = 1.0f)
    {
        const float half_size = size / 2.0f;
    ...
    inline GeometryData gen_sphere_geometry(
        const float radius = 0.5f,
        const size_t segments = 32,
        const size_t rings = 16,
